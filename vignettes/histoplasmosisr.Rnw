...
\title{Histoplasmosis data for specified animals}
...
@
<<get_loc_data, echo = FALSE, include = FALSE>>=
#possible_durations <- c("month", "year", "day", "week")
#timeframe <- "2 weeks" # c("1 year", "2 year", "1 month", "30 days")
#date_num <- as.numeric(unlist(strsplit(timeframe, " "))[1])
#date_str <- possible_durations[str_detect(timeframe, possible_durations)]
<<create_ed_hist>>=
X_ed_hist <- "X_ed_hist"
...
par(mfrow = c(1, 2))
cat("\\begin{figure}\n")
hist(df$day_of_year, main = '', xlab = "Day of Year")
hist(df$month, main = '', xlab = "Month of Year")
...
cat(str_c("\\caption{Histogram of number of \\emph{Histoplasmosis duboisii} ",
          ...
          "on ",
          strftime(min(df$first_noted), format = '%D'), " and the last case ",
          "seen on ", strftime(max(df$first_noted), format = '%D'), ".}\n"))
cat("\\end{figure}\n")
...
affected_males <- 
  length(male_female_ratio_df$sex[male_female_ratio_df$sex == 'M'])
affected_females <- 
  length(male_female_ratio_df$sex[male_female_ratio_df$sex == 'F'])
total_males <- sum(male_female_ratio_df$males)
total_females <- sum(male_female_ratio_df$females)
chisq_sex <- chisq.test(x = matrix(c(affected_males,
                                     affected_females,
                                     total_males,
                                     total_females), nrow = 2))
odbcClose(wt_conn)

@
<<print-sex-ratio-results, results='as.is', include = TRUE>>=
There were \Sexpr{affected_males} affected males and \Sexpr{affected_females}
in the \Sexpr{nrow(df)} animals with Histoplasmosis. We counted the number of
...
was a 
\Sexpr{ifelse(df$sex[1] == 'M', "male", "female")}
