...
## This next line is needed because controls are not found for all animals 
...
daily_exp_df <- make_daily_df(conn, X_exp, exp_df, housing_types)
daily_ctrl_df <- make_daily_df(conn, X_ctrl, ctrl_df, housing_types)
