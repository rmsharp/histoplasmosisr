...
require(histoplasmosis_lb, quietly = TRUE)
@
...
for a differential risk (increase in susceptibility to histoplasmosis)
between the sexes was to compare the ratio of affected males 
(\Sexpr{affected_males}) to affected females (\Sexpr{affected_females})
to the ratio of the sum of all males present on the first day each animal was
noted to have Histoplasmosis 
(\Sexpr{total_males}) and the sum of all females present on the 
first day each animal was noted to have Histoplasmosis (\Sexpr{total_females})
using a chi-square analysis with Yates' continuity correction. The probability
that there is no difference in the ratios is 
\Sexpr{signif(chisq_sex$p.value, 4)}.
