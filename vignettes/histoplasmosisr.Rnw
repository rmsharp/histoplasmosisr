...
Hello, 
I got an email from Terry regarding TAC, so I'll get that set up.  
...
If I come across any other terms/forms, I'll let you know)
...
higher occurrence of histo.  I'm not sure how to relate kinship to this.
...
This might be a start.  I'll come by and check in with you tomorrow.  T
...
exp_df <- exp_df[exp_df$id %in% ctrl_df$affected_id, ]
add_match_id <- function() {
  exp_df <- exp_df[order(exp_df$id), ]
  ctrl_df <- ctrl_df[order(ctrl_df$match_id), ]
  exp_df$match_id <- ctrl_df$id
}
