...
daily_df <- make_daily_df(conn, X_ed_hist)

@
<<get_df_percentages, echo = FALSE, include = TRUE, results = "asis">>=
df <- add_location_type_percents(df, daily_df)
...
cat(str_c("\\caption{Histogram of number of \\emph{Histoplasmosis duboisii} 
          ...
          strftime(min(no_roundup_df$first_noted), format = '%D'), 
          " and the last case seen on ", 
          strftime(max(no_roundup_df$first_noted), format = '%D'), 
          ". The ", nrow(roundup_animals_df), " animals that were 
...
# it
other_locations <- unique(daily_df$location[daily_df$gang == 0 & daily_df$corral == 0 
                                          & daily_df$single == 0])
...
\item Create a matrix where each row represents \Sexpr{ntrials} animals 
corresponding to one of the incident cases and there is a row for all incident
